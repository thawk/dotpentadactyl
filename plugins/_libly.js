...
<VimperatorPlugin>
    ...
    <detail><![CDATA[
== Objects ==
- dactyl.plugins.libly.$U
- dactyl.plugins.libly.Request
- dactyl.plugins.libly.Response
- dactyl.plugins.libly.Wedata
...
libly.$U = {//{{{
    ...
    getLogger: function(prefix) {
        return new function() {
            this.log = function(msg, level) {
                ...
                dactyl.log(libly.$U.dateFormat(new Date()) + ': ' + (prefix || '') + ': ' + msg, (level || 0));
            };
    ...
    around: (function () {
        function getPluginPath () {
          ...
          Error('hoge').stack.split(/\n/).some(
            function (s)
              let (m = s.match(/^\(\)@chrome:\/\/dactyl\/content\/dactyl\.js -> (.+):\d+$/))
                (m && (pluginPath = m[1]))
          );
          return pluginPath;
        }
        ...
        return function (obj, name, func, autoRestore) {
            ...
            if (autoRestore) {
                ...
                if (pluginPath) {
                    ...
                    dactyl.echoerr('getPluginPath failed');
                }
            }
            original = obj[name];
    ...
    eval: function(text) {
        ...
        } catch (e) { dactyl.log('warning: _libly.js is working with unsafe sandbox.'); }
        ...
        try {
            ...
            if (logins.length) {
                if (username) {
                    ...
                    dactyl.log(this.dateFormat(new Date()) +': [getUserAndPassword] username notfound');
                ...
                dactyl.log(this.dateFormat(new Date()) + ': [getUserAndPassword] account notfound');
                return [];
            }
        } catch (e) {
            dactyl.log(this.dateFormat(new Date()) + ': [getUserAndPassword] error: ' + e, 0);
            return null;
        }
    },
    // }}}
