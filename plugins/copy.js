...
EOM
    ...
    <item>
        ...
        <description>
            <code><ex>options.copy_use_wedata = false; // false by default</ex></code>
            <p>true に設定すると wedata からテンプレートを読込みます。</p>
            <code><ex>options.copy_wedata_include_custom = true; // false by default</ex></code>
            ...
            <code><ex>options.copy_wedata_exclude_labels = ['pathtraqnormalize', ];</ex></code>
...
copy_templates = options.copy_templates.map(function(t){
...
group.commands.add(['copy'],'Copy to clipboard',
    function(args){
...
function addUserMap(label, map){
    group.mappings.add([modes.NORMAL,modes.VISUAL], map,
        label,
...
function wedataRegister(item){
    ...
    if (item.custom && item.custom.toLowerCase().indexOf('function') != -1) {
        if (!options.copy_wedata_include_custom ||
             item.label == 'test') {
...
var exCopyManager = {
    ...
    copy: function(arg, special, appendMode){
        ...
        if (appendMode){
            copyString = dactyl.clipboardRead() + copyString;
        }
...
if (options.copy_use_wedata){
    function loadWedata(){
        ...
        if (options.copy_wedata_exclude_labels)
            options.copy_wedata_exclude_labels.forEach(function(item) excludeLabelsMap[item] = 1);
